...
The variable [[latex:progstruct][org-babel-load-languages]] should
...
defining new link types with [[latex:progstruct][org-add-link-type]].  Two new links are
...
command.  The [[latex:package][hyperref]] package asks to be loaded last; it is loaded
here =AtBeginDocument=, which follows on the other packages loaded
=AtEndOfClass=.
...
[[latex:progstruct][org-export-latex-listings]] to =listings= ensures that source code
blocks in langauges that aren't listed in
[[latex:progstruct][org-export-latex-custom-lang-environments]] are also typeset by the
...
[[latex:progstruct][org-export-latex-custom-lang-environments]].  Note, too, that the names
