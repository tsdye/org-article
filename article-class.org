...
Instructions for using [[latex:package][tabularx]] with [[latex:proglang][Org-mode]] are [[tabularx_package][here]].  The method
used to export tables involves tangling a file with the table code and
then using a line like =#+LaTeX:\input{filename}= to read the table code
