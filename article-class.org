...
#+LaTeX_CLASS_OPTIONS: [koma,listof=flat,letterpaper,fontset=bera,11pt,source=listings,microtype,lists=inparaa,colorlinks=true,urlcolor=SteelBlue4,linkcolor=Firebrick4,secnum=section,gantt,tabularx,captions=tableheading,bib,natbib,hyperref]
...
This file also uses an [[latex:progstruct][abstract]] environment, which you can see [[abstract-environment][here]].
...
[[latex:classfile][Org-article]] defines a [[latex:progstruct][caption]] link type that overcomes these various
limitations.  Using a link like =[caption:To list][With figure.]= will
export to LaTeX as =\caption[To list]{With figure.}=.  Note that you
will have to enter these captions by hand rather than with
[[latex:progstruct][org-insert-link]].  That function is designed to handle spaces in paths
and it inserts =%20= in their stead, which don't make it through LaTex.

*** Citations
There are very many bibliography options in LaTeX.  [[latex:classfile][Org-article]] is set
up to deal with the most popular variants.  By default it uses plain
BibTeX, with its =\cite= command.  Using the [[latex:progstruct][bib]] key and passing the
value =natbib=, the various citation commands defined by this package
are made available.  If the =biblatex= value is passed to the [[latex:progstruct][bib]] key,
then the citation commands of what will soon be the preferred LaTeX
bibliography option will be made available.

In any case, entering a citation uses the extended link syntax, e.g.,
=[citep:jones][423]=.  This would export to LaTeX as
=\citep[423]{jones}= and would result in a parenthetical citation
using the [[latex:package][natbib]] package. 

If =ebib= is set up on your system, then this process can be automated
with lookup in the bibliography database and the link, when clicked,
will take you to the entry in the bibliographic database.

[[citep:aaa_diaconis:_theor_of_data_analy][citation]]
...
#+begin_src latex :exports code
  ...
  {\AtEndOfClass{\RequirePackage{url}
...
[[latex:classfile][Org-article]] supports the [[latex:package][biblatex]] bibliobraphy package, which can be
specified with the =bibliography= key (Listing
\ref{fig:bibliography-option}).  The [[latex:package][biblatex]] package, which recently emerged
from beta status, offers support for the full range of citation
styles, including the numeric styles favored by scientists, the
author-date styles used by social scientists, and the footnote styles
popular in the humanities.


#+BEGIN_listing
#+source: option-bibliography
#+begin_src latex :exports code
  \newboolean{ORGART@bib}
  \DeclareOption{bib}{%
    \setboolean{ORGART@bib}{true}}
#+end_src
[[caption:The bibliography option][The bib option.]]
# <<fig:bibliography-option>>
#+END_listing

#+source: pass-to-biblatex
#+begin_src latex :exports none
  \DeclareOption{style}{%
    \PassOptionsToPackage{style}{biblatex}}
  \DeclareOption{bibstyle}{%
    \PassOptionsToPackage{bibstyle}{biblatex}}
  \DeclareOption{citestyle}{%
    \PassOptionsToPackage{citestyle}{biblatex}}
  \DeclareOption{natbib}{%
    \PassOptionsToPackage{natbib}{biblatex}}
  \DeclareOption{sorting}{%
    \PassOptionsToPackage{sorting}{biblatex}}
  \DeclareOption{sortcase}{%
    \PassOptionsToPackage{sortcase}{biblatex}}
  \DeclareOption{sortupper}{%
    \PassOptionsToPackage{sortupper}{biblatex}}
  \DeclareOption{sortlocale}{%
    \PassOptionsToPackage{sortlocale}{biblatex}}
  \DeclareOption{sortlos}{%
    \PassOptionsToPackage{sortlos}{biblatex}}
  \DeclareOption{sortcites}{%
    \PassOptionsToPackage{sortcites}{biblatex}}
  \DeclareOption{maxnames}{%
    \PassOptionsToPackage{maxnames}{biblatex}}
  \DeclareOption{minnames}{%
    \PassOptionsToPackage{minnames}{biblatex}}
  \DeclareOption{maxitems}{%
    \PassOptionsToPackage{maxitems}{biblatex}}
  \DeclareOption{minitems}{%
    \PassOptionsToPackage{minitems}{biblatex}}
  \DeclareOption{autocite}{%
    \PassOptionsToPackage{autocite}{biblatex}}
  \DeclareOption{autopunct}{%
    \PassOptionsToPackage{autopunct}{biblatex}}
  \DeclareOption{language}{%
    \PassOptionsToPackage{language}{biblatex}}
  \DeclareOption{clearlang}{%
    \PassOptionsToPackage{clearlang}{biblatex}}
  \DeclareOption{babel}{%
    \PassOptionsToPackage{babel}{biblatex}}
  \DeclareOption{block}{%
    \PassOptionsToPackage{block}{biblatex}}
  \DeclareOption{notetype}{%
    \PassOptionsToPackage{notetype}{biblatex}}
  \DeclareOption{hyperref}{%
    \PassOptionsToPackage{hyperref}{biblatex}}
  \DeclareOption{backref}{%
    \PassOptionsToPackage{backref}{biblatex}}
  \DeclareOption{backrefstyle}{%
    \PassOptionsToPackage{backrefstyle}{biblatex}}
  \DeclareOption{style}{%
    \PassOptionsToPackage{style}{biblatex}}
  \DeclareOption{backrefsetstyle}{%
    \PassOptionsToPackage{backrefsetstyle}{biblatex}}
  \DeclareOption{indexing}{%
    \PassOptionsToPackage{indexing}{biblatex}} 
  \DeclareOption{loadfiles}{%
    \PassOptionsToPackage{loadfiles}{biblatex}}
  \DeclareOption{refsection}{%
    \PassOptionsToPackage{refsection}{biblatex}}
  \DeclareOption{refsegment}{%
    \PassOptionsToPackage{refsegment}{biblatex}}
  \DeclareOption{citerest}{%
    \PassOptionsToPackage{citerest}{biblatex}}
  \DeclareOption{abbreviate}{%
    \PassOptionsToPackage{abbreviate}{biblatex}}
  \DeclareOption{origdate}{%
    \PassOptionsToPackage{origdate}{biblatex}}
  \DeclareOption{eventdate}{%
    \PassOptionsToPackage{eventdate}{biblatex}}
  \DeclareOption{urldate}{%
    \PassOptionsToPackage{urldate}{biblatex}}
  \DeclareOption{alldates}{%
    \PassOptionsToPackage{alldates}{biblatex}}
  \DeclareOption{datezeros}{%
    \PassOptionsToPackage{datezeros}{biblatex}}
  \DeclareOption{dateabbrev}{%
    \PassOptionsToPackage{dateabbrev}{biblatex}}
  \DeclareOption{defernumbers}{%
    \PassOptionsToPackage{defernumbers}{biblatex}}
  \DeclareOption{punctfont}{%
    \PassOptionsToPackage{punctfont}{biblatex}}
  \DeclareOption{arxiv}{%
    \PassOptionsToPackage{arxiv}{biblatex}}
  \DeclareOption{backend}{%
    \PassOptionsToPackage{backend}{biblatex}}
  \DeclareOption{bibencoding}{%
    \PassOptionsToPackage{bibencoding}{biblatex}}
  \DeclareOption{bibwarn}{%
    \PassOptionsToPackage{bibwarn}{biblatex}}
  \DeclareOption{mincrossrefs}{%
    \PassOptionsToPackage{mincrossrefs}{biblatex}}
  \DeclareOption{useauthor}{%
    \PassOptionsToPackage{useauthor}{biblatex}}
  \DeclareOption{useeditor}{%
    \PassOptionsToPackage{useeditor}{biblatex}}
  \DeclareOption{usetranslator}{%
    \PassOptionsToPackage{usetranslator}{biblatex}}
  \DeclareOption{useprefix}{%
    \PassOptionsToPackage{useprefix}{biblatex}}
  \DeclareOption{spaces}{%
    \PassOptionsToPackage{spaces}{url}}
  \DeclareOption{hyphens}{%
    \PassOptionsToPackage{hyphens}{url}}
  \DeclareOption{obeyspaces}{%
    \PassOptionsToPackage{obeyspaces}{url}}
#+end_src


#+source: load-bibliography
#+begin_src latex :exports code
  \ifthenelse{\boolean{ORGART@bib}}
  {\AtEndOfClass{\RequirePackage{biblatex}
      \bibliography{tsd}}}
  {}
#+end_src

*** Natbib citation commands
[[latex:classfile][Org-article]] supports the various citation commands implemented by
[[latex:package][natbib]] (Listings
\ref{fig:define-natbib-links}--\ref{fig:citetstar-link}).  To use
these, you will need to pass the option =natbib=true= to toggle the
[[latex:package][biblatex]] package's emulation of the [[latex:package][natbib]] commands.  The commands all
start with =cite= and end with one or more letters that indicate a
style.  The most commonly used commands are:
...
[[caption:Define the natbib link types][Define the natbib link types.]]
# <<fig:define-natbib-links>>
#+END_listing
...
#+end_src
[[caption:The citep link type][The citep link type.]]
# <<fig:citep-link>>
#+END_listing
...
[[caption:The citet link type][The citet link type.]]
# <<fig:citet-link>>
#+END_listing
...
#+end_src
[[caption:The citealt link type][The citealt link type.]]
# <<fig:citealt-link>>
#+END_listing
...
#+end_src
[[caption:The citealp link type][The citealp link type.]]
# <<fig:citealp-link>>
#+END_listing
...
[[latex:classfile][Org-article]] supports the six citation commands standard in the
[[latex:package][biblatex]] package (Listings
\ref{fig:define-biblatex-link-types}--\ref{fig:cap-footcite-link}).
Certain styles define other citation commands.  The user can easily
extend [[latex:classfile][Org-article]] by adding new link types for these citation
commands.

#+BEGIN_listing
#+source: biblatex-links
#+begin_src emacs-lisp :noweb yes
  <<define-cite-link>>
  <<define-parencite-link>>
  <<define-footcite-link>>
  <<define-cap-cite-link>>
  <<define-cap-parencite-link>>
  <<define-cap-footcite-link>>
#+end_src
[[caption:Define%20the%20biblatex%20link%20types][Define the biblatex link types.]]
# <<fig:define-biblatex-link-types>>
#+END_listing
...
#+end_src
[[caption:The cite link type][The cite link type.]]
# <<fig:cite-link>>
#+END_listing
...
#+end_src
[[caption:The parencite link type][The parencite link type.]]
# <<fig:parencite-link>>
#+END_listing
...
#+end_src
[[caption:The footcite link type][The footcite link type.]]
# <<fig:footcite-link>>
#+END_listing
...
the box for HTML.
** DONE Semantic markup for blocks, Christian Moe
   :LOGBOOK:
   - State "DONE"       from "TODO"       [2010-12-12 Sun 07:27]
   :END:
...
** TODO Ebib link
   SCHEDULED: <2010-12-12 Sun>
For some reason, the links aren't expanding with latex export.  I
fixed this earlier by getting rid of :results silent, I think.  Now,
it isn't working again.  Curious, because the link types like caption
work just fine.  Perhaps it is because they are added over and over again?
...
This is now a citation [[citet:aaa_king_districts][135]]
Now you can insert ebib links in your documents like this:
[[ebib:aaa_king:_districts][some paper title]]. Opening this link should now
