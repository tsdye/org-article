...
Mathematical Society's [[http://www.ams.org/publications/authors/tex/amsfonts][symbol fonts]] =msam= and =msbm=.  This package
is superseded by the =mathdesign= package.  They are
...
to use this mechanism because there is no reason to assume that it
will be stable.  Instead, options for this package should be processed
by =org-article.cls= itself using a =key = val= interface that sets values
using the =listings= package's =lstset= function.
...
The options are set using the =xkeyval= package.  Note that the family
argument is used in the declaration, in this case =<listings>=.  This
makes it easier to process the options later.

The option =lst-basicstyle= sets the size of the font for the
listings.  It takes a LaTeX font size command.

#+source: lst-basicstyle
#+begin_src latex :exports none
\DeclareOptionX<listings>{lst-basicstyle}[small]{\lstset{basicstyle = \#1}  
#+end_src
