...
the class file and export to LaTeX.  Variables have been made
buffer-local, where possible, so you should find your environment
relatively intact when you exit the buffer.  Exceptions to this
general statement include [[special%20blocks][org-special-blocks]], which is loaded to wrap
code blocks in LaTeX environments, and the two link types defined for
[[in-line%20markup][in-line markup]] of semantic elements.  These will remain in your setup.
...
#+source: format-toc
#+begin_src emacs-lisp :exports code
  (make-local-variable 'org-export-latex-format-toc-function)
...
#+source: minted-setup
#+begin_src emacs-lisp :exports code :results silent
  (make-local-variable 'org-export-latex-listings)
  (make-local-variable 'org-export-latex-minted-lang-environments)
  (make-local-variable 'org-latex-to-pdf-process)
