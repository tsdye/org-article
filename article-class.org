...
#+LaTeX_CLASS_OPTIONS: [koma,letterpaper,times,11pt,listings-es,microtype,paralist,colorlinks=true,urlcolor=blue,secnums,gantt]
...
The following source code block was sent to the Org-mode list by Eric
Fraga.  It should be installed in the Library of Babel.

#+source: elispgantt
#+begin_src emacs-lisp :var table=gantttesttable :results output raw :exports code
...
#+call: elispgantt(table=gantttesttable)

#+results: elispgantt(table=gantttesttable)
